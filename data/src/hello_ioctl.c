...
static long device_ioctl(struct file *filp, unsigned int ioctl_num, unsigned long ioctl_param)
{
	...
	if (ioctl_num == PWN_GET && strncmp(message, "PASSWORD", sizeof(message)) == 0) {
		...
		is_copy_invalid = copy_from_user(message, (char *)ioctl_param, 16);
                ...
                return ioctl_num == PWN_GET ? -EACCES : -ENOTTY;
	}
...
int init_module(void)
{
	...
	if (IS_ERR(flag_fd))
		return PTR_ERR(flag_fd);
	result = kernel_read(flag_fd, flag, sizeof(flag) - 1, &offset);
